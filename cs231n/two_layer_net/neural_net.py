...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        first_neurons = X.dot(W1) + b1
        first_activ = np.maximum(first_neurons, 0)
        scores = first_activ.dot(W2) + b2
        ...
        exp_true = exp_prob[range(N), y]
        exp_quot = exp_true/exp_sum
        per_element_loss = -np.log(exp_quot)

        loss = np.sum(per_element_loss)/N  + reg * (np.sum(W1 ** 2) + np.sum(W2 ** 2))
